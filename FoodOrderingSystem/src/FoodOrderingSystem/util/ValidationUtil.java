...
public class ValidationUtil {
    ...
    private static final String EMAIL_REGEX = "^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,6}$";    private static final String PHONE_REGEX = "^[6-9]\\d{9}$";
    private static final String ID_REGEX = "^[A-Z0-9-]{3,15}$";
    private static final String PASSWORD_REGEX = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{6,}$";
    private static final String SIMPLE_PASS_REGEX = "^[a-zA-Z0-9]{6,20}$";
    ...
    public static String getValidPassword(Scanner sc) {
        while (true) {
            String input = sc.nextLine().trim();
            if (Pattern.matches(SIMPLE_PASS_REGEX, input)) {
                return input;
            }
            System.out.println("Invalid... Password must be alphanumeric and 6-20 characters long.");
